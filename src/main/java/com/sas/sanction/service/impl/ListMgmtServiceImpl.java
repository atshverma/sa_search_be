...
public class ListMgmtServiceImpl implements ListMgmtService {
	...
	public List<ListMgmtResponseDto> searchListData(ListMgmtSearchRequestDto listMgmtSearchDto) {
		...
		if(shouldSearch){
			...
			if(SanctionUtils.isStringNotNullorEmpty(listMgmtSearchDto.getListType())){
				 listTypes = listMasterDao.findByListType(listMgmtSearchDto.getListType());
			}
			
			ListMgmtDataSpec listMgmtDataSpec = new ListMgmtDataSpec(listMgmtSearchDto, listTypes);
